...
app.get('/api/health', (c)=>c.json({ data:{ status:'ok', service:'xxxignal', phase:2 } }))

const api = new Hono<AppEnv>()
api.use('*', async (c,next)=>{
  if (!['GET','HEAD','OPTIONS'].includes(c.req.method)) {
    const origin=c.req.header('origin'); const host=c.req.header('host')
    if (origin && host) { try { if (new URL(origin).host !== host) throw new AppError(403,'origin_not_allowed','許可されていないOriginからのリクエストです。') } catch(error){ if(error instanceof AppError) throw error; throw new AppError(403,'origin_not_allowed','Originヘッダーを検証できませんでした。') } }
  }
  const session=await ensureSession(c.env,c.req.raw); c.set('session',session); await next()
})
